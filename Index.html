...
/* ============================================================
   ...
   Production build — single-file React app
   ============================================================
   LOGO: expects /k31-logo.png next to the built app
         (the file is included in the same outputs folder)

   FIREBASE INTEGRATION POINTS (search "FIREBASE:")
   ============================================================ */

const LOGO_SRC = "/k31-logo.png"; // drop k31-logo.png in /public

/* Palette pulled directly from the logo */
const THEME = {
  primary:     "#1BA5FF",  // electric blue (wings / "MOBILE")
  primaryDeep: "#0066D6",  // cobalt shadow
  primaryGlow: "#4FC3FF",  // light-blue speed streak
  accent:      "#FF7A1F",  // orange flame
  accentGlow:  "#FFB347",  // orange highlight
  accentDeep:  "#E85A10",

  chromeHi:    "#F4F4F7",  // silver highlight
  chromeMid:   "#C7C7CF",  // chrome mid
  chromeLo:    "#6E6E78",  // chrome shadow

  bg:          "#05060A",  // near-black
  bgGlow:      "#0C1220",  // blue-tinted dark
  surface:     "#11131B",
  surface2:    "#1A1D27",
  border:      "#242836",
  text:        "#F5F7FA",
...
/* ============================================================
   BRAND COMPONENTS — real logo, no placeholder text mark
   ============================================================ */

function BrandLogo({ size = 140, className = "" }) {
  // Full horizontal logo for auth / onboarding / splash
  return (
    <img
      src={LOGO_SRC}
      alt="K31 Mobile Tire Shop"
...
function BrandLogoCompact({ size = 44 }) {
  // Small logo for headers / nav / bottom sheet — constrained square frame
  return (
    <div
      className="k31-logo-compact"
      style={{ width: size, height: size }}
      aria-label="K31 Mobile Tire Shop"
    >
...
export default function App() {
  ...
  const [settings, setSettings] = useState({
    perMileRate: 1.25, freeMiles: 5, heavyDutyMultiplier: 1.6, phone: "",
  });
  ...
  const bumpSavedLocation = useCallback((loc) => {
    if (!loc) return;
    setSavedLocations(prev => {
      const existing = prev.find(p => p.name.toLowerCase() === loc.toLowerCase());
      if (existing) {
        return prev.map(p => p.name.toLowerCase() === loc.toLowerCase()
          ? { ...p, count: p.count + 1, lastUsed: Date.now() }
          : p);
      }
      return [...prev, { name: loc, count: 1, lastUsed: Date.now() }];
    });
  }, []);
  ...
  const saveJob = useCallback((jobData) => {
    ...
    if (jobData.id) {
      setJobs(prev => prev.map(j => j.id === jobData.id ? { ...j, ...jobData, updatedAt: Date.now() } : j));
    } else {
      const newJob = { ...jobData, id: uid(), createdAt: Date.now(), updatedAt: Date.now() };
  ...
  const updateJobStatus = useCallback((id, status) =>
    setJobs(prev => prev.map(j => j.id === id ? { ...j, status, updatedAt: Date.now() } : j)), []);

  if (!user) return <AuthScreen onAuth={(u) => setUser(u)} />;
  if (!onboarded) return <Onboarding profile={profile} setProfile={setProfile} onDone={() => setOnboarded(true)} />;
  ...
  if (tab === "new" || editingJobId) {
    ...
    return (
      <JobForm
        ...
        onSave={(data) => { saveJob(data); setEditingJobId(null); setTab("jobs"); }}
      />
    );
  }
  ...
  if (viewingJobId) {
    ...
    return (
      <JobDetail
        ...
        onDelete={() => { deleteJob(job.id); setViewingJobId(null); }}
      ...
      <main className="k31-main">
        ...
        {tab === "settings" && (
          <SettingsScreen
            ...
            user={user}
            onLogout={() => { setUser(null); setOnboarded(false); }}
          />
        )}
      </main>
...
function AuthScreen({ onAuth }) {
  ...
  const googleSignIn = async () => {
    ...
    await new Promise(r => setTimeout(r, 400));
    setLoading(false);
    onAuth({ email: "driver@k31.com", uid: uid() });
  };
  ...
  return (
    <div className="k31-auth">
      ...
      <div className="k31-auth-card">
        <div className="k31-auth-logo">
          <BrandLogo size={240} />
        </div>
        <div className="k31-auth-tagline">
          Mobile Tire · Roadside Assistance
        </div>
...
function Onboarding({ profile, setProfile, onDone }) {
  ...
  const canNext = () => {
    if (step === 0) return profile.businessName.trim().length > 1;
    if (step === 1) return profile.serviceArea.trim().length > 1;
  ...
  return (
    <div className="k31-onboard">
      ...
      <div className="k31-onboard-head">
        <div className="k31-onboard-logo">
          <BrandLogo size={110} />
        </div>
      ...
      <div className="k31-onboard-body">
        {step === 0 && (
          <div className="k31-field">
            ...
            <input className="k31-input" value={profile.businessName}
        ...
        {step === 1 && (
          <div className="k31-field">
            ...
            <CitySearchInput value={profile.serviceArea}
                ...
                return (
                  <button key={s.id} type="button" className={`k31-service-card ${on ? "on" : ""}`}
                    onClick={() => {
                      ...
                      setProfile({ ...profile, services: nxt });
                    }}>
                    <Icon size={22} />
            ...
            <div className="k31-pricing-choices">
              {[
                ...
                <button key={p.id} type="button" className={`k31-pricing-card ${profile.pricingPreference === p.id ? "on" : ""}`}
                  onClick={() => setProfile({ ...profile, pricingPreference: p.id })}>
      ...
      <div className="k31-onboard-foot">
        ...
        <button className="k31-btn-primary" onClick={next} disabled={!canNext()}>
          {step === steps.length - 1 ? "Finish Setup" : "Continue"} <ChevronRight size={18} />
        </button>
      </div>
    </div>
  );
...
function JobForm({ existing, profile, settings, savedLocations, onCancel, onSave }) {
  ...
  const canSave = serviceId && location.trim().length > 0 && finalPrice >= 0;
...
${settings.phone || ""}`;
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: `Invoice ${invoiceNum}`, text });
        setSent(true);
      } catch (e) { /* cancelled */ }
    } else {
      try {
      ...
      <div className="k31-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="k31-invoice">
          ...
          <table className="k31-invoice-table">
            <thead>
              <tr><th>Service</th><th style={{ textAlign: "right" }}>Amount</th></tr>
            </thead>
            <tbody>
              <tr>
                <td>
...
/* ============================================================
   SETTINGS
   ============================================================ */
function SettingsScreen({ profile, setProfile, settings, setSettings, user, onLogout }) {
  return (
    <div className="k31-page">
      <header className="k31-header">
        <div className="k31-header-brand">
      ...
      <section className="k31-card">
        ...
        <div className="k31-field-inline">
          ...
          <input className="k31-input" value={profile.businessName} onChange={e => setProfile({ ...profile, businessName: e.target.value })} />
        </div>
        <div className="k31-field-inline">
          <label className="k31-label">Service Area</label>
          <CitySearchInput value={profile.serviceArea} onChange={v => setProfile({ ...profile, serviceArea: v })} />
        </div>
        <div className="k31-field-inline">
      ...
      <section className="k31-card">
        ...
        <div className="k31-row-2">
          <div className="k31-field-inline">
            ...
            <input className="k31-input" type="number" step="0.05" value={settings.perMileRate}
              onChange={e => setSettings({ ...settings, perMileRate: Number(e.target.value) || 0 })} />
          </div>
          <div className="k31-field-inline">
            <label className="k31-label">Free miles</label>
            <input className="k31-input" type="number" value={settings.freeMiles}
              onChange={e => setSettings({ ...settings, freeMiles: Number(e.target.value) || 0 })} />
          </div>
        </div>
        ...
        <div className="k31-field-inline">
          ...
          <input className="k31-input" type="number" step="0.1" min="1" value={settings.heavyDutyMultiplier}
            onChange={e => setSettings({ ...settings, heavyDutyMultiplier: Math.max(1, Number(e.target.value) || 1) })} />
      ...
      <div className="k31-version">K31 v1.0 · Production Build</div>
    </div>
  );
}
...
/* ============================================================
   GLOBAL STYLES — refined to match logo palette
   ============================================================ */
  ...
  return (
    <style>{`
      :root {
        ...
        --k-primary-deep: ${THEME.primaryDeep};
        --k-primary-glow: ${THEME.primaryGlow};
        --k-accent: ${THEME.accent};
        --k-accent-glow: ${THEME.accentGlow};
        --k-accent-deep: ${THEME.accentDeep};
        --k-chrome-hi: ${THEME.chromeHi};
        --k-chrome-mid: ${THEME.chromeMid};
        --k-chrome-lo: ${THEME.chromeLo};
        --k-bg: ${THEME.bg};
        --k-bg-glow: ${THEME.bgGlow};
        --k-surface: ${THEME.surface};
        --k-surface2: ${THEME.surface2};
        --k-border: ${THEME.border};
        --k-text: ${THEME.text};
        --k-dim: ${THEME.textDim};
        --k-mute: ${THEME.textMute};
      ...
      * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
      ...
      .k31-biz {
        font-size: 18px; font-weight: 800; margin: 2px 0 0;
        letter-spacing: -.01em;
        background: linear-gradient(180deg, var(--k-chrome-hi) 0%, var(--k-chrome-mid) 50%, var(--k-chrome-lo) 100%);
      ...
      .k31-logo-img {
        display: block;
        filter: drop-shadow(0 4px 22px rgba(27, 165, 255, .35))
                drop-shadow(0 2px 8px rgba(255, 122, 31, .2));
      }
      .k31-logo-compact {
      ...
      .k31-btn-primary {
        ...
        background: linear-gradient(135deg, var(--k-primary-glow) 0%, var(--k-primary) 40%, var(--k-primary-deep) 100%);
        ...
        transition: transform .1s, opacity .1s, box-shadow .2s; min-height: 48px;
        letter-spacing: .01em;
      }
      .k31-btn-primary:active { transform: scale(.98); box-shadow: 0 3px 12px rgba(27, 165, 255, .4); }
      .k31-btn-primary:disabled { opacity: .4; cursor: not-allowed; box-shadow: none; }

      .k31-btn-ghost {
        background: rgba(255,255,255,.02); color: var(--k-text);
        border: 1px solid var(--k-border);
      ...
      .k31-link { background: transparent; border: 0; color: var(--k-primary);
        font-size: 13px; font-weight: 600; cursor: pointer;
        display: inline-flex; align-items: center; gap: 2px; }
      .k31-link-btn { background: transparent; border: 0; color: var(--k-accent);
        font-size: 12px; font-weight: 600; cursor: pointer; padding: 4px 0; }

      /* ---- INPUTS ---- */
      .k31-label {
        display: block; font-size: 11px; font-weight: 700;
        color: var(--k-dim); margin-bottom: 6px;
      ...
      .k31-input:focus, .k31-textarea:focus {
        border-color: var(--k-primary);
        box-shadow: 0 0 0 3px rgba(27, 165, 255, .18);
      }
      ...
      .k31-err {
        background: rgba(239, 68, 68, .12); color: var(--k-danger);
      ...
      .k31-card {
        background: linear-gradient(180deg, var(--k-surface) 0%, #0F1119 100%);
        border: 1px solid var(--k-border);
        border-radius: 18px; padding: 18px; margin-bottom: 14px;
      }
      .k31-sec-title {
        font-size: 11px; font-weight: 800; color: var(--k-dim);
      ...
      @keyframes streak {
        0% { opacity: .4; transform: translateX(-3%); }
      ...
      .k31-auth-card {
        ...
        border: 1px solid rgba(27, 165, 255, .15);
        backdrop-filter: blur(24px);
        border-radius: 28px; padding: 32px 26px 26px;
      ...
      .k31-auth-tagline {
        ...
        color: var(--k-dim); letter-spacing: .2em;
        text-transform: uppercase; margin-bottom: 24px;
      }

      .k31-auth-tabs {
        display: grid; grid-template-columns: 1fr 1fr;
        background: var(--k-surface2); border-radius: 12px; padding: 4px;
        margin-bottom: 18px; border: 1px solid var(--k-border);
      }
      .k31-auth-tab {
        background: transparent; color: var(--k-dim); border: 0;
        padding: 10px; border-radius: 9px; font-size: 14px; font-weight: 600;
      ...
      .k31-auth-divider {
        ...
        margin: 18px 0; color: var(--k-mute); font-size: 11px;
        text-transform: uppercase; letter-spacing: .1em;
      }
      .k31-auth-divider::before, .k31-auth-divider::after {
        content: ""; flex: 1; height: 1px; background: var(--k-border);
      }
      ...
      .k31-progress-dot {
        height: 4px; flex: 1; background: var(--k-border); border-radius: 2px;
        transition: background .3s;
      }
      .k31-progress-dot.active {
        background: linear-gradient(90deg, var(--k-primary-glow), var(--k-primary));
      }
      ...
      .k31-step-title {
        ...
        background: linear-gradient(180deg, var(--k-chrome-hi) 0%, var(--k-chrome-mid) 100%);
        -webkit-background-clip: text; background-clip: text;
        -webkit-text-fill-color: transparent;
      }
      ...
      .k31-week-card {
        background:
          ...
          linear-gradient(180deg, #141724 0%, #0E1018 100%);
        position: relative; overflow: hidden;
      ...
      .k31-week-card::before {
        ...
        background: radial-gradient(circle, rgba(27,165,255,.25), transparent 70%);
        pointer-events: none;
      }
      .k31-week-top { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 18px; position: relative; }
      .k31-week-label {
        font-size: 11px; font-weight: 700; color: var(--k-primary-glow);
      ...
      .k31-chip.on {
        background: linear-gradient(135deg, var(--k-primary-glow), var(--k-primary));
        color: white; border-color: var(--k-primary);
        box-shadow: 0 2px 10px rgba(27, 165, 255, .35);
      }
      ...
      .k31-tag-hd {
        ...
        color: var(--k-accent-glow);
        padding: 2px 6px; border-radius: 4px;
        border: 1px solid rgba(255, 122, 31, .3);
      }
      .k31-job-meta {
        font-size: 11px; color: var(--k-dim); margin-top: 2px;
      ...
      .k31-svc-pick.on {
        border-color: var(--k-primary);
        background: linear-gradient(135deg, rgba(27,165,255,.15), rgba(27,165,255,.05));
        color: var(--k-primary-glow);
      }

      .k31-toggle-row {
        display: flex; justify-content: space-between; align-items: center;
        padding: 12px 0; margin-top: 8px; border-top: 1px solid var(--k-border); gap: 12px;
      }
      ...
      .k31-toggle {
        width: 44px; height: 26px; background: var(--k-surface2);
      ...
      .k31-price-line {
        display: flex; justify-content: space-between; align-items: center;
        padding: 7px 0; font-size: 13px; color: var(--k-dim); gap: 10px;
      }
      .k31-price-line.dim { opacity: .6; }
      .k31-price-sug {
        border-top: 1px dashed var(--k-border); margin-top: 4px; padding-top: 10px;
        color: var(--k-text); font-weight: 700; display: flex; align-items: center;
      }
      ...
      .k31-price-final {
        border-top: 1px solid var(--k-border); padding-top: 10px;
        font-size: 16px; font-weight: 800; color: var(--k-text);
      }
      .k31-price-inline {
        background: var(--k-surface); border: 1px solid var(--k-border);
        color: var(--k-text); border-radius: 8px; padding: 4px 8px;
        font-size: 13px; width: 90px; text-align: right; outline: none;
        font-family: inherit;
      }
      ...
      .k31-final-row {
        display: flex; justify-content: space-between; align-items: center;
        background:
          linear-gradient(135deg, rgba(27,165,255,.18), rgba(255,122,31,.08));
      ...
      .k31-profit-chip {
        ...
        background: rgba(34, 197, 94, .12); color: var(--k-success);
      ...
      .k31-status-pick {
        background: var(--k-surface2); border: 1.5px solid var(--k-border);
        color: var(--k-dim); border-radius: 10px; padding: 10px 14px;
        font-size: 13px; font-weight: 600; cursor: pointer;
      ...
      .k31-info-row {
        display: flex; align-items: center; gap: 8px;
        padding: 10px 0; border-bottom: 1px solid var(--k-border); font-size: 14px;
      }
      ...
      .k31-invoice-logo {
        width: 68px; height: auto; flex-shrink: 0;
      ...
      .k31-invoice-table th {
        text-align: left; padding: 10px 0; border-bottom: 2px solid #1a1a1a;
      ...
      .k31-search-item {
        width: 100%; background: transparent; border: 0; color: var(--k-text);
      ...
      .k31-empty {
        ...
        background: var(--k-surface); border: 1px dashed var(--k-border);
        border-radius: 18px;
      }
      .k31-empty-icon {
        width: 56px; height: 56px; margin: 0 auto 14px;
        border-radius: 16px; background: var(--k-surface2);
        display: flex; align-items: center; justify-content: center;
        color: var(--k-dim);
      }
      ...
      .k31-logout {
        ...
        width: 100%; background: transparent; color: var(--k-danger);
      ...
      .k31-nav-btn {
        background: transparent; border: 0; color: var(--k-mute);
        padding: 8px 0; cursor: pointer; font-size: 10px; font-weight: 700;
      ...
      .k31-fab {
        ...
        background:
          radial-gradient(circle at 30% 30%, var(--k-accent-glow), var(--k-accent) 55%, var(--k-accent-deep));
